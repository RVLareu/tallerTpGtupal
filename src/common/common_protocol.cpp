...
void Protocol::send_board_status(Socket& socket,
    ...
    if (!game.is_running){
        vector_board.push_back('f'); //(f)inished
        if (game.is_whites_turn()){
            vector_board.push_back('w'); //(w)hites
        } else{
            vector_board.push_back('b'); //(b)lacks
        }
    } else{
        // Se obtienen los posibles movimientos de la pieza seleccionada actualmente (de existir)      
        if (game.board.is_any_piece_selected()){
            std::tuple<int, int> selected_pos = game.board.get_selected_piece_position();
...
void Protocol::recv_client_events(Socket& socket, Game& game){    
    ...
    this->send_board_status(socket, game);
}
